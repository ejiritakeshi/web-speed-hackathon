...
export const useSlider = ({ items }: { items: unknown[] }) => {
  ...
  const [visibleItemCount, _setVisibleItemCount] = useState(1);
  const [_slideIndex, setSlideIndex] = useState(0);
  const slideIndex = Math.min(Math.max(0, _slideIndex), items.length - 1);

  useEffect(() => {
    const setVisibleItemCount = () => {
      const containerWidth = containerElementRef.current?.getBoundingClientRect().width ?? 0;
      _setVisibleItemCount(Math.max(Math.floor(containerWidth / ITEM_MIN_WIDTH), 1));
    };
    setVisibleItemCount();

    window.addEventListener('resize', setVisibleItemCount);

    return () => {
      window.removeEventListener('resize', setVisibleItemCount);
    }
  }, []);
